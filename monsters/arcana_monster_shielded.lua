...
function init()
  ...
  self.shieldHealthPercent = config.getParameter("shieldHealthPercent", 0)
  self.shieldHealth = (self.shieldHealthPercent * status.resource("health"))
  self.shieldStatus = config.getParameter("shieldStatus", "arcana_status_shielded_aurea")
  self.baseStatus = config.getParameter("baseStatus", false)
  ...
  self.damageTaken = damageListener("damageTaken", function(notifications)
    for _,notification in pairs(notifications) do
      if notification.healthLost > 0 then
	    --sb.logInfo("[DEBUG] ---DAMAGED---")
	    --sb.logInfo("[DEBUG] damage:" .. notification.damageDealt)
		--sb.logInfo("[DEBUG] health lost:" .. notification.healthLost)
		--sb.logInfo("[DEBUG] health:" .. status.resource("health"))
	    --sb.logInfo("[DEBUG] shield health:" .. self.shieldHealth)
		
		if self.shieldHealth > 0 then
		  status.modifyResource("health", notification.healthLost)
		  self.shieldHealth = self.shieldHealth - notification.healthLost
		  if self.shieldHealth <= 0 then
		    status.clearPersistentEffects("shielded")
			animator.burstParticleEmitter("shieldBreak")
			animator.playSound("shieldBreak")
		  end
		  --sb.logInfo("[DEBUG] ---APPLY SHIELD---")
		  --sb.logInfo("[DEBUG] health:" .. status.resource("health"))
	      --sb.logInfo("[DEBUG] shield health:" .. self.shieldHealth)
		end
        self.damaged = true
  ...
  if self.shieldHealth > 0 then
    status.setPersistentEffects("shielded", {self.shieldStatus})
  end
  if self.baseStatus then
    status.addEphemeralEffect(self.baseStatus or "arcana_status_kill", config.getParameter("baseStatusDuration") or 6)
  end
end
...
function shouldDie()
  return (self.shouldDie and status.resource("health") <= 0 and self.shieldHealth <= 0) or capturable.justCaptured
end
