...
function updateBonus(id)
  local hasSet = false
  if status.stat(self.set) >= self.minParts and queryHealth(50) == true then hasSet = true end
...
function queryHealth(radius)
  local monsters = world.monsterQuery(entity.position(), radius)
  local max = 0
  if not next(monsters) then return false end 
  for _, monster in pairs(monsters) do 
    if world.entityHealth(monster)[1] > max then max = world.entityHealth(monster)[1] end
  end
  if max > world.entityHealth(entity.id())[1] then return true else return false end
end