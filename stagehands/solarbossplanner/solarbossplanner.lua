...
function init()
  ...
  storage.elementalType = config.getParameter("elementalType", storage.elementalType or util.randomFromList({"fire"}))
...
function bossParametersFromPlan(bossAbilities, plan, state, spaceConfig)
  ...
  if vulnerability == "PiercingVulnerability" then
    ...
    bossParameters.statusSettings.stats.protection.baseValue = 0.0
    bossParameters.statusSettings.stats.maxHealth.baseValue = 5500.0
  ...
  if #sequenceActions > 0 then
    ...
    if #sequenceActions > 4 then
      table.insert(phaseSequence.parameters.actions, 1, {
          ...
          parameters = {
            maxHealth = lastPhaseHealth,
            actions = util.takeEnd(sequenceActions, 5)
          }
        })
      lastPhaseHealth = 0.5
    end
...
function spawnBoss(bossParameters, bossUid, seed)
  ...
  return world.spawnMonster(string.format("arcana_boss_solarGuardian", storage.elementalType), entity.position(), bossParameters)
end
