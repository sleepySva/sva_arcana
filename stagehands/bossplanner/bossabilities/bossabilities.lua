...
function solarminionTriggerGroupHandler(ability, operation, plan, state, spaceConfig)
  local behaviorConfig = ability.behaviorConfig
  local triggerId = minionTriggerId(operation, plan)
  local minionType = util.find(operation.postconditions, function(term) return term[1] == "Minion" end)[2][1]
  ability = mapAbilityActions(ability, function(action)
      if minionType == "kill" then
        action.parameters.spawns = {
          {
            monsterType = "arcana_monster_solisummoned",
            position = vec2.add({-12, 12}, entity.position()),
            parameters = {
              managerUid = entity.uniqueId(),
              triggerId = triggerId,
              partOfGroup = true
      ...
      elseif minionType == "collide" then
        ...
        action.parameters.spawns = {
          {
            monsterType = "arcana_monster_tricanardsummoned",
            positions = positions,
            parameters = {
              managerUid = entity.uniqueId(),
              triggerId = triggerId,
...
function solarspawnMinionGroupHandler3(ability, operation, plan, state, spaceConfig)
  self.rand = self.rand or sb.makeRandomSource(storage.seed)
  local platforms = platformPositions(state, spaceConfig)
  while #platforms < 4 do
    local spaces = {{self.rand:randUInt(0, spaceConfig.dimensions[1] - 1), self.rand:randUInt(1, spaceConfig.dimensions[2] - 1)}}
...
function solarspawnMinionGroupHandler2(ability, operation, plan, state, spaceConfig)
  self.rand = self.rand or sb.makeRandomSource(storage.seed)
  local platforms = platformPositions(state, spaceConfig)
  while #platforms < 4 do
    local spaces = {{self.rand:randUInt(0, spaceConfig.dimensions[1] - 1), self.rand:randUInt(1, spaceConfig.dimensions[2] - 1)}}
  ...
  ability = mapAbilityActions(ability, function(action)
      ...
      action.parameters.spawns = {
        {
          monsterType = string.format("arcana_monster_tricanardsummoned", storage.elementalType),
          positions = platforms,
          parameters = {
            managerUid = entity.uniqueId(),
            attack = attackConfig,
...
function solarspawnMinionGroupHandler(ability, operation, plan, state, spaceConfig)
  self.rand = self.rand or sb.makeRandomSource(storage.seed)
  local platforms = platformPositions(state, spaceConfig)
  while #platforms < 4 do
    local spaces = {{self.rand:randUInt(0, spaceConfig.dimensions[1] - 1), self.rand:randUInt(1, spaceConfig.dimensions[2] - 1)}}
  ...
  ability = mapAbilityActions(ability, function(action)
      ...
      action.parameters.spawns = {
        {
          monsterType = string.format("arcana_monster_solsticesummoned", storage.elementalType),
          positions = platforms,
          parameters = {
            managerUid = entity.uniqueId(),
            attack = attackConfig,
