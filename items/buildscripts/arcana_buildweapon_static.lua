...
function build(directory, config, parameters, level, seed)
  ...
  -- build element swap directives
  if builderConfig.elementPalette then
    local palette = root.assetJson(util.absolutePath(directory, builderConfig.elementPalette))
    local selectedSwaps = palette.swaps[elementalType]
    for k, v in pairs(selectedSwaps) do
      config.paletteSwaps = string.format("%s;%s=%s", config.paletteSwaps, k, v)
    end
  end
  ...
  if builderConfig.gunParts then
    ...
    for _,part in ipairs(builderConfig.gunParts) do
      ...
      imageOffset = vec2.add(imageOffset, {0, 0})
