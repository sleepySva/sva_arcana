...
function init()
  self.crewType = config.getParameter("crewType", "janitor")
  self.crewLevel = config.getParameter("crewLevel", 1)
  self.crewSpecies = config.getParameter("crewSpecies", {"human"})
...
function update(dt, fireMode, shiftHeld)
  ...
  if storage.firing and animator.animationState("firing") == "off" then
    item.consume(1)
    if player then
      summon(self.crewSpecies, self.crewType, 6)
    end
...
function summon(races, type, level)
  local race = races[math.random(util.tableSize(races))]
  world.spawnNpc(mcontroller.position(), race, type, level)
end
