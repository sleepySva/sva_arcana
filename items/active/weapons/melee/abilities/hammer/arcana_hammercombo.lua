...
function MeleeCombo:fire()
  ...
  animator.setSoundPitch(animStateKey, 1.0 + (math.random(-1,1)/10))
  animator.playSound(animStateKey)
  
  -- Impact
  if self.comboStep == self.comboSteps then
    animator.setSoundPitch("groundImpact", 1.0 + (math.random(-1,1)/10))
	animator.playSound("groundImpact")
	if self.impact == true then
	  local position = vec2.add(mcontroller.position(), activeItem.handPosition(self.impactOffset))
	  local params = {
