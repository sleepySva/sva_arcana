...
function MeleeCombo:fire()
  ...
  if self.comboStep == 3 then
    ...
    if self.blinkPosition and status.overConsumeResource("energy", 20) then
...
function MeleeCombo:slashAction()
  blinkburst()
  ...
  self.blinkPosition = vec2.add(mcontroller.position(), {self.blinkDistance * mcontroller.facingDirection(), 5})

  status.removeEphemeralEffect("arcana_blinkfast")
  blinkburst()
  mcontroller.setPosition(self.blinkPosition)
  ...
  util.wait(self.stances.slash.duration, function(dt)
    ...
    self.weapon:setDamage(self.damageConfig, damageArea)
      ...
      for _, timeAndAngle in pairs(projectileTimesAndAngles) do
        if timeAndAngle[1] <= dt then
          self:spawnProjectile(-45)
        else
          table.insert(newTimesAndAngles, {timeAndAngle[1] - dt, -45})
        end
      end
      projectileTimesAndAngles = newTimesAndAngles
  end)
  
  self.blinkPosition = vec2.add(mcontroller.position(), {self.blinkDistance * mcontroller.facingDirection(), 0})
...
function MeleeCombo:findBlinkPosition()
...
function blinkburst()
  status.addEphemeralEffect("arcana_blinkfast")
  util.wait(0.1)
end