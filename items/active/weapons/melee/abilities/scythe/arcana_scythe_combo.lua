...
function MeleeCombo:windup()
  ...
  if stance.flip then animator.setGlobalTag("flipDirectives", self.flipDirectives or "flipx") else animator.setGlobalTag("flipDirectives", "") end
  self.edgeTriggerTimer = 0
  
  local chargeTimer = 0
  local chargeTime = stance.duration or 0.3
  while chargeTimer < chargeTime do
    if stance.crouch then mcontroller.controlCrouch() end
...
function MeleeCombo:wait()
  ...
  local skipWait = stance.skipWait

  self.weapon:setStance(stance)
  if stance.flip then animator.setGlobalTag("flipDirectives", self.flipDirectives or "flipx") else animator.setGlobalTag("flipDirectives", "") end

  util.wait(stance.duration, function()
    if self:shouldActivate() or skipWait then
      self:setState(self.windup)
      return
    end
  end)

  self.cooldownTimer = math.max(0, self.cooldowns[self.comboStep - 1] - stance.duration - self.cooldownReduction)
  self.comboStep = 1
...
function MeleeCombo:fire()
  ...
  animator.setSoundPitch(animStateKey, 1.0 + (math.random(-2,2)/10))
  ...
  elseif self.comboStep == 3 then
	comboMove({20 * self.weapon.aimDirection, 0}, 1)
  elseif self.comboStep == 4 then
	comboMove({-20 * self.weapon.aimDirection, 0}, 1)
  end
  
  if self.impact == true and self.comboStep == 4 then
    for _, offset in pairs(self.impactOffsets) do
	  local position = vec2.add(mcontroller.position(), activeItem.handPosition(offset))
	  local params = {
	    powerMultiplier = activeItem.ownerPowerMultiplier(),
	    power = self.impactDamage * config.getParameter("damageLevelMultiplier", 1.0)
	  }
      world.spawnProjectile(self.impactProjectile, position, activeItem.ownerEntityId(), self:aimVector(), false, params)
	end
  end
  ...
  if self.comboStep == 3 or self.comboStep == 4 then mcontroller.setVelocity({0,0}) end
  ...
  if self.stances["after"..self.comboStep] then
    ...
    if self.comboStep < self.comboSteps then
      ...
      self.cooldownTimer = math.max(0, self.cooldowns[self.comboStep] - self.cooldownReduction)
	  self.comboStep = 1
    end
  end
end
...
function MeleeCombo:aimVector()
  ...
  aimVector = {aimVector[1]*-1 , aimVector[2]*-1}
  return aimVector
end
...
function MeleeCombo:computeDamageAndCooldowns()
  ...
  for i, attackTime in ipairs(attackTimes) do
    ...
    self.stepDamageConfig[i].baseDamage = damageFactor * self.baseDps * (self.fireTime + 0.2)

    totalAttackTime = totalAttackTime + attackTime
    totalDamageFactor = totalDamageFactor + damageFactor

    local targetTime = totalDamageFactor * (self.fireTime + 0.2)
...
function comboMove(momentum, force)
  mcontroller.setVelocity(momentum)
end
