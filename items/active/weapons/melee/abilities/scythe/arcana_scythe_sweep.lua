...
function MeleeCombo:init()
  ...
  self.dashControlForce = config.getParameter("dashControlForce", 2000)
...
function MeleeCombo:fire()
  ...
  local momentum = {self.dashSpeed * 100, 0}
  ...
  util.wait(stance.duration, function()
	...
	mcontroller.controlApproachVelocity({(vec2.rotate({1, 0}, self.weapon.aimAngle))[1] * momentum[1] * self.weapon.aimDirection, (vec2.rotate({1, 0}, self.weapon.aimAngle))[2] * momentum[2]}, self.dashControlForce)
