...
function update(dt)
	if mcontroller ~= nil then
		...
		if self.InitData == false then
			...
			world.spawnProjectile("GrandCrossZanaNormalBulletShot" .. math.random(1,2), mcontroller.position(), entity.id(), CalculateRotate, true, 
				{
			...
			for Idx,Bullet in pairs(self.BulletStack) do
				if Bullet.Delay <= 0 then
					...
					world.spawnProjectile("GrandCrossZanaNormalBullet", mcontroller.position(), entity.id(), CalculateRotate, false, 
						{
